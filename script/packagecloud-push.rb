...
package_files = Dir.glob("/tmp/orchestrator-agent-release/**/*.rpm") + Dir.glob("/tmp/orchestrator-agent-release/**/*.deb")
package_files.each do |full_path|
  ...
  distro_names.map do |distro_name|
    ...
    result = $client.put_package(packagecloud_repo, pkg, distro_id)
...
package_files.each do |full_path|
  ...
  next unless (full_path =~ /\/orchestrator-agent[-|_]\d/)
  ...
  puts "[#{os}](https://packagecloud.io/#{packagecloud_user}/#{packagecloud_repo}/packages/#{distro}/#{File.basename(full_path)}/download)"
end
